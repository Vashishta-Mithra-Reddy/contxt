...
export default function Dashboard({ session }: { session: any }) {
  ...
  // NEW: ensure the dialog auto-opens only once after projects finish loading
  const hasOpenedInitially = React.useRef(false);
  ...
  // FIX: move auto-open logic from render to an effect to prevent re-render loops
  React.useEffect(() => {
    if (!loading && projects.length === 0 && !hasOpenedInitially.current) {
      setDialogOpen(true);
